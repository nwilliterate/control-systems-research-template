...
def test_simulate_shapes_and_zero_torque(robot_and_cfg):
    """simulate returns consistent (t, x, tau) shapes; zero torque -> zero tau."""
    robot, cfg = robot_and_cfg
    x0 = np.concatenate([cfg.initial_state.q, cfg.initial_state.v])
    none_ctrl = lambda t, q, v: np.zeros(robot.nv)
    t, x, tau = simulate(robot, none_ctrl, x0, 0.5, cfg.simulation.dt)
    assert x.shape == (len(t), robot.nq + robot.nv)
    assert tau.shape == (len(t) - 1, robot.nv)
    assert np.allclose(tau, 0.0)


@pytest.mark.parametrize("ctype", ["computed_torque", "lqr", "pid"])
def test_controllers_regulate_to_setpoint(robot_and_cfg, ctype):
    """Each config-selected controller drives the double pendulum toward q_des."""
    robot, _ = robot_and_cfg
    cfg = load_config("config/params.yaml")  # fresh, mutable copy
    cfg["controller"]["type"] = ctype

    q_des = np.zeros(robot.nv)
    ctrl = make_controller(cfg, robot, q_des, dt=cfg.simulation.dt)
    x0 = np.concatenate([[0.3, -0.2], [0.0, 0.0]])
    t, x, tau = simulate(robot, ctrl, x0, 4.0, cfg.simulation.dt)
    final_q_err = np.linalg.norm(x[-1, : robot.nq] - q_des)
